...
module.exports = function(program, directory, stage, webpackPort, routes) {
    ...
    function module(config) {
      ...
      config.loader('images',{ test: /\.(jpe?g|png|gif|svg)$/i, loader: 'url?limit=10000!img?progressive=true' })
      ...
      switch(stage) {
        case "develop":
          config.loader('css', {
            test: /\.css$/,
            loaders: ['style', `css${cssLoaderConfig}`, 'postcss']
          });
          config.merge({
            postcss: [
        ...
        case "production":
          config.loader('css', {
            test: /\.css$/,
            loaders: [`css/locals${cssLoaderConfig}`, 'postcss']});
            config.loader('less',{ test: /\.less/, loaders: ['style', 'css', 'less']});
